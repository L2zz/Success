...
const site = "https://cscience.skku.edu/cscience/community/total_notice.do"
const compSite = site + "?mode=list&&articleLimit=10&article.offset=0"
const getHtml = async() => {
  try {
...
getHtml().then(html => {
  ...
  var lastRef = db.ref("site/5/category/0/last");
  var lastId;
  lastRef.once('value').then(function(snapshot) {
    ...
    for (var i=0; i<ulList.length; i++) {
      ...
      var articleRef = db.ref("site/5/category/0/article").push();
