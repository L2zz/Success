...
const site = "https://shb.skku.edu/semicon/menu_5/comm_04.jsp"
const compSite = site + "?mode=list&board_no=1789&pager.offset=0"
const getHtml = async() => {
  try {
...
getHtml().then(html => {
  ...
  const $bodyList = $("table.list_table tbody").children("tr");
  ...
  var lastRef = db.ref("site/2/category/0/last");
  var lastId;
  lastRef.once('value').then(function(snapshot) {
    ...
    return lastId;
      ...
      var articleRef = db.ref("site/2/category/0/article").push();
