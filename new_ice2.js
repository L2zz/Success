...
const compSite =  site + "board_list_square?listPage=1&boardName=board_seminar&field=subject&keyword="
const getHtml = async() => {
  try {
...
getHtml().then(html => {
  ...
  var lastRef = db.ref("site/1/category/2/last");
  var lastId;
  lastRef.once('value').then(function(snapshot) {
    ...
    return lastId;
      ...
      var articleRef = db.ref("site/1/category/2/article").push();
